...
# Unified DQMC engine for the 1D Hubbard model with long-range hopping.
# NN is the special case Rmax=1: t(1) = t/1^a = t for any a.
#
# Function signature:
#   run_hubbard_chain_simulation(sID, U, μ, β, L, a, Rmax, N_burnin, N_updates, N_bins)
#
# Output directory format:
#   hubbard_U{U:.2f}_mu{μ:.2f}_b{β:.2f}_L{L}_a{a:.2f}_Rmax{Rmax}-{sID}

function run_hubbard_chain_simulation(sID, U, μ, β, L, a, Rmax, N_burnin, N_updates, N_bins; filepath = ".")

    # Construct the foldername the data will be written to.
    datafolder_prefix = @sprintf "hubbard_U%.2f_mu%.2f_b%.2f_L%d_a%.2f_Rmax%d" U μ β L a Rmax
    ...
    # Define long-range bonds for the 1D chain up to range Rmax.
    # For Rmax=1 this reduces to pure nearest-neighbor hopping (t(1) = t/1^a = t).
    bonds = lu.Bond{1}[]
    bond_ids = Int[]
    tvals = Float64[]

    for r in 1:Rmax
        bond = lu.Bond(orbitals = (1,1), displacement = [r])
        bond_id = add_bond!(model_geometry, bond)
        push!(bonds, bond)
        push!(bond_ids, bond_id)
        push!(tvals, t / r^a)  # hopping amplitude decays as t/r^a
    end

    # Define the tight-binding model
    tight_binding_model = TightBindingModel(
        model_geometry = model_geometry,
        t_bonds = bonds,    # defines hopping
        t_mean = tvals,     # defines corresponding hopping amplitudes
    ...
    # Apply Ising Hubbard-Stratonovich (HS) transformation, and initialize
    # corresponding HS fields that will be sampled in the DQMC simulation.
    ...
    # time_displaced=false, integrated=true: gives the Matsubara-summed G(r),
    # which is the correct observable for equal-time pair correlation analysis.
    ...
    # calculating their respective determinants at the same time.
    ...
    # Green's function matrices corrected by numerical stabilization.
    ...
    for n in 1:N_burnin
        ...
        # Perform a sweep through the lattice, attempting an update to each Ising HS field.
    ...
    # Iterate over the number of bins, i.e. the number of times measurements will be dumped to file.
    for bin in 1:N_bins
    ...
    # Record the maximum numerical error corrected by numerical stabilization.
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # Command-line arguments (10 total):
    #   sID U μ β L a Rmax N_burnin N_updates N_bins
    # Example (small test run):
    #   julia simulation/hubbard_dqmc.jl 1 -5 -2.2 15 16 2.0 1 100 200 10
    sID      = parse(Int,     ARGS[1])  # simulation ID
    U        = parse(Float64, ARGS[2])  # on-site interaction
    μ        = parse(Float64, ARGS[3])  # chemical potential
    β        = parse(Float64, ARGS[4])  # inverse temperature
    L        = parse(Int,     ARGS[5])  # lattice size
    a        = parse(Float64, ARGS[6])  # power-law exponent (alpha)
    Rmax     = parse(Int,     ARGS[7])  # max hopping range (1 = NN)
    N_burnin = parse(Int,     ARGS[8])  # thermalization sweeps
    N_updates = parse(Int,    ARGS[9])  # measurement sweeps
    N_bins   = parse(Int,     ARGS[10]) # number of bins
